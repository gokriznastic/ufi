...
def display_triplet_distance(model,train_loader,name):
    ...
    for batch_idx, (data_a, data_p, data_n,c1,c2) in enumerate(train_loader):
        ...
        try:
            ...
            data_a_v, data_p_v, data_n_v = Variable(data_a_c), \
                                    Variable(data_p_c), \
                                    Variable(data_n_c)
        ...
        print("Distance (anchor-positive): {}".format(l2_dist.forward(out_a,out_p)[0].item()))
        print("Distance (anchor-negative): {}".format(l2_dist.forward(out_a,out_n)[0].item()))


        axarr[0].imshow(denormalize(data_a[0]))
        axarr[1].imshow(denormalize(data_p[0]))
        axarr[2].imshow(denormalize(data_n[0]))
        axarr[0].set_title("Distance (anchor-positive): {}".format(l2_dist.forward(out_a,out_p)[0].item()))
        axarr[2].set_title("Distance (anchor-negative): {}".format(l2_dist.forward(out_a,out_n)[0].item()))
...
def display_triplet_distance_test(model,test_loader,name):
    ...
    for batch_idx, (data_a, data_n, label) in enumerate(test_loader):
        ...
        try:
            ...
            data_a_v, data_n_v = Variable(data_a_c), \
                                    Variable(data_n_c)
        ...
        for i in range(5):
            ...
            distance = l2_dist.forward(out_a,out_n)[rand_index].item()
